...
Feature: UploadNewCandidateProfile_po @feature_upload_candidate_profile
  ...
  @valid-candidate-profile-upload
  Scenario Outline: Upload New Candidate Profile
    When the user clicks on the upload (cloud) icon
    And the user browses and selects the <candidate_profile_file>
    And the user clicks on the Upload button
    Then the <candidate_profile_file> uploads successfully with a confirmation message
...
Scenario Outline: Upload New Candidate Profile
  ...
  And the recruiter browses and selects a candidate profile file with metadata
...
Examples:
  ...
  Scenario Outline: Upload New Candidate Profile
    When the user clicks on the upload (cloud) icon
    And the user browses and selects a candidate profile file with a valid extension <file_extension>
    And the user clicks on the Upload button
    And the user waits for the upload process to complete
    Then a success confirmation message appears, indicating the profile was uploaded successfully
...
Examples:
  ...
  Scenario Outline: Upload New Candidate Profile
    When the user clicks on the upload (cloud) icon
    And the user browses and selects a candidate profile file in <file_format> format
    And the user clicks on the Upload button
    And the user waits for the upload process to complete
    Then a success confirmation message should appear, indicating the profile was uploaded successfully
  ...
  Scenario Outline: Upload New Candidate Profile
    ...
    And all uploaded profiles should be listed in the Profile section
